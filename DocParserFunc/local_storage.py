...
class LocalFileSaver:
    ...
    def save_processing_results(
        ...
        try:
            ...
            if self.config.save_metadata:
                metadata_file = self._save_metadata(doc_folder, metadata, processing_stats, document_id, filename, timing_stats)
                saved_files["metadata"] = str(metadata_file)
                logger.info(f"Saved metadata: {metadata_file}")
            
            # Save timing stats separately
            if timing_stats:
                timing_file = self._save_timing_stats(doc_folder, timing_stats)
                saved_files["timing"] = str(timing_file)
                logger.info(f"Saved timing stats: {timing_file}")
            
            # Save summary
            summary_file = self._save_summary(doc_folder, document_id, filename, len(chunks), len(embeddings), saved_files, timing_stats)
    ...
    def _save_timing_stats(self, doc_folder: Path, timing_stats: Dict[str, Any]) -> Path:
        """Save timing statistics as separate JSON file."""
        timing_file = doc_folder / "timing_stats.json"
        
        with open(timing_file, 'w', encoding='utf-8') as f:
            json.dump(timing_stats, f, indent=2, ensure_ascii=False)
        
        logger.debug(f"Saved timing stats to {timing_file}")
        return timing_file
...
"""
        ...
        if timing_stats:
            summary_content += f"""Timing Performance:
- Total processing time: {timing_stats.get('total_processing_time', 0):.2f}s
- PDF processing: {timing_stats.get('pdf_processing_time', 0):.2f}s
- Parallel processing: {timing_stats.get('parallel_processing_time', 0):.2f}s
- Search index update: {timing_stats.get('search_index_time', 0):.2f}s

"""
        
        summary_content += "Files created:\n"
